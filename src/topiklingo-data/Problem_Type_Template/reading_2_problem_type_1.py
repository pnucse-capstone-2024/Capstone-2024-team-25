...
def generate_reading_2_problem_type_1(verbose=False, default_model="gpt-4o-mini"):
    total_cost = 0
    ...
    try:
        ...
        random_question = find_random_question(
            keyword, problem_type, detail_type=1, count=2
        )
        if verbose:
        ...
        problem_generate_model.request_models_responses(
            [
                SystemMessage(
                    content="#예시처럼 주어지는 주제어에 대한 빈칸 문법 맞추기 문제 1개를 생성해줘. #답변 예시 "
                    + example_prompt
                ),
                HumanMessage(content="주제어: " + word),
            ]
        )
        problem_responses = problem_generate_model.get_model_responses()
        total_cost += float(
            re.search(r"Total Cost \(KRW\): ₩([\d.]+)", problem_responses[0][2]).group(
                1
            )
        )
        if verbose:
