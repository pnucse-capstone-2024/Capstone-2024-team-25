...
def generate_reading_2_problem_type_14(verbose=False, default_model="gpt-4o-mini"):
    total_cost = 0
    ...
    for example in example_dict:
        ...
        if question_number==1:
            ...
            example_str += (
                "올바른 핵심 내용 요약 찾기 문제" +" 선택지: " + example_selector_str
            )
    ...
    selector_generate_model.request_models_responses(
            [
                SystemMessage(
                    content="너는 한국어 문제를 만드는 선생님이야. 주어지는 문장을 이용해 예시같은 선다형 문제를 만들어줘. 각 선택지는 긴 어절로 구성해줘. 단, 정답 이외의 선택지는 문장과 어울리지 않는 선택지여야 해." + "예시: " + example_str
                ),
...
if __name__ == "__main__":
    result = generate_reading_2_problem_type_14(default_model="gpt-4o-mini",verbose=True)
    print(result)